...
def main() -> None:
    ...
    project = args.core.resolve()
    if not (project / "test/release/benchmark_optimization.py").is_file():
        parser.error("--core must contain the Core release training and benchmark workloads")
    toolchain = Path(__file__).resolve().parents[1]
    helper = toolchain / "scripts/toolchain/build-library-variant.py"
    ...
    if args.operation == "sdk":
        ...
        execute(
            ...
            [
                "cmake",
                "-S",
                str(toolchain / "tests/integration"),
                "-B",
                str(consumer),
                "-G",
                "Ninja",
