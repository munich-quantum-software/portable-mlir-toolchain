...
def test_failed_command_and_replay(tmp_path: Path) -> None:
    ...
    runner = Path(__file__).resolve().parents[2] / "experiments/linux_optimization.py"
...
def test_windows_records_failures_without_linux_resource_claims(tmp_path: Path) -> None:
    ...
    runner = runpy.run_path(str(Path(__file__).resolve().parents[2] / "experiments/linux_optimization.py"))
...
def test_sdk_archive_with_two_gib_window(tmp_path: Path) -> None:
    ...
    subprocess.run(
        [
            sys.executable,
            str(Path(__file__).resolve().parents[2] / "experiments/extract_study_sdk.py"),
            str(archive),
            str(destination),
        ],
        check=True,
...
def test_downloaded_wheels_keep_separate_identities(tmp_path: Path, *, damaged: bool) -> None:
    ...
    benchmark = source / "test/release/benchmark_optimization.py"
    benchmark.parent.mkdir(parents=True)
    benchmark.write_text("training fixture")
    ...
    result = subprocess.run(
        [
            sys.executable,
            str(project / "experiments/evaluate_optimization_study.py"),
            "--core",
            str(source),
            "--downloads",
            str(tmp_path),
...
def test_equal_family_weight_and_paired_intervals(tmp_path: Path) -> None:
    ...
    evaluator = Path(__file__).resolve().parents[1] / "evaluate_optimization.py"
...
def test_matched_sdk_gate_uses_best_native_and_every_cohort(tmp_path: Path) -> None:
    ...
    subprocess.run(
        [
            sys.executable,
            str(Path(__file__).resolve().parents[1] / "decide_optimization.py"),
            *map(str, paths),
            "--native",
            "native",
            "--native",
